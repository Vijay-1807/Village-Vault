...
class AIService {
  ...
  constructor() {
    ...
    // Prefer Direct OpenAI if enabled
    if (AI_CONFIG.OPENAI_DIRECT && AI_CONFIG.OPENAI_DIRECT.ENABLED) {
      defaultModel = AI_CONFIG.OPENAI_DIRECT.MODEL
    } else if (AI_CONFIG.GEMINI_DIRECT.ENABLED) {
      // Direct Gemini if OpenAI is not enabled
      defaultModel = AI_CONFIG.GEMINI_DIRECT.MODEL
    }

    this.config = {
      apiKey: (AI_CONFIG.OPENAI_DIRECT && AI_CONFIG.OPENAI_DIRECT.ENABLED) ? AI_CONFIG.OPENAI_DIRECT.API_KEY : AI_CONFIG.API_KEY,
      baseUrl: (AI_CONFIG.OPENAI_DIRECT && AI_CONFIG.OPENAI_DIRECT.ENABLED) ? AI_CONFIG.OPENAI_DIRECT.BASE_URL : AI_CONFIG.BASE_URL,
      model: defaultModel,
  ...
  switchModel(modelName: string) {
    ...
    // Update config to OpenRouter if switching to a fallback model (assumed to be OpenRouter)
    // IMPORTANT: If we started with Direct OpenAI, switching models implies falling back to OpenRouter
    // so we need to update the base URL and API Key to OpenRouter ones if we move away from the direct model
    if (modelName !== AI_CONFIG.OPENAI_DIRECT.MODEL && modelName !== AI_CONFIG.GEMINI_DIRECT.MODEL) {
      this.config.baseUrl = AI_CONFIG.BASE_URL; // OpenRouter Base URL
      this.config.apiKey = AI_CONFIG.API_KEY;   // OpenRouter API Key
    }
  }
  ...
  getAvailableModels() {
    ...
    // Add Direct OpenAI first if enabled
    if (AI_CONFIG.OPENAI_DIRECT && AI_CONFIG.OPENAI_DIRECT.ENABLED) {
      models.push(AI_CONFIG.OPENAI_DIRECT.MODEL)
    }

    // Add Direct Gemini Pro if enabled
  ...
  private async tryWithFallback(message: string, context?: string): Promise<AIResponse> {
    ...
    try {
      ...
      for (let i = 0; i < models.length && attempts < maxAttempts; i++) {
        const nextModel = models[i];

        // Skip current model if it's the one that just failed
        // Logic fix: make sure we are not retrying the exact same configuration
        if (nextModel === this.config.model && i === currentModelIndex) continue;
        if (nextModel === this.config.model && i !== currentModelIndex) continue; // Should not happen if index is correct

        attempts++
        try {
          // Switch model configuration (handles changing API keys/URLs)
          this.switchModel(nextModel);

          console.log(`Trying fallback model: ${nextModel} (attempt ${attempts}/${maxAttempts})`)
          ...
          console.log(`Fallback successful with ${nextModel}`)
          return result
        } catch (fallbackError) {
          ...
          console.log(`Fallback model ${nextModel} also failed: ${fallbackErrorMsg}`)
...
*Note: This is a fallback response while services are restored.*`
  ...
  private async makeAPICall(message: string, context?: string): Promise<AIResponse> {
    // Check if using direct OpenAI API
    if (AI_CONFIG.OPENAI_DIRECT && AI_CONFIG.OPENAI_DIRECT.ENABLED && this.config.model === AI_CONFIG.OPENAI_DIRECT.MODEL) {
      return await this.makeDirectOpenAICall(message, context);
    }
  ...
  // Make API call to Direct OpenAI API
  private async makeDirectOpenAICall(message: string, context?: string): Promise<AIResponse> {
    ...
    const response = await fetch(`${AI_CONFIG.OPENAI_DIRECT.BASE_URL}/chat/completions`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${AI_CONFIG.OPENAI_DIRECT.API_KEY}`,
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        model: AI_CONFIG.OPENAI_DIRECT.MODEL,
        messages: [
          {
            role: 'system',
    ...
    if (!response.ok) {
      const errorMessage = `OpenAI API request failed: ${response.status}`
      console.error(`Direct OpenAI failed with status ${response.status}`)

      if (response.status === 429) {
        throw new Error(`Rate limit exceeded (429) for OpenAI`)
      } else if (response.status === 400) {
        throw new Error(`Bad request (400) for OpenAI`)
      } else if (response.status === 401 || response.status === 403) {
        throw new Error(`Unauthorized (${response.status}) - OpenAI API key issue`)
      } else {
  ...
  private async makeDirectGeminiAPICall(message: string, context?: string): Promise<AIResponse> {
    ...
    // Use gemini-1.5-flash for speed and reliability as fallback
    const modelName = 'gemini-1.5-flash'
    // Gemini often works best with v1beta/models/...
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${AI_CONFIG.GEMINI_DIRECT.API_KEY}`
    ...
    if (!response.ok) {
      ...
      console.error(`Direct Gemini failed with status ${response.status}`)

      if (response.status === 429) {
        throw new Error(`Rate limit exceeded (429) for Gemini`)
      } else if (response.status === 400) {
        throw new Error(`Bad request (400) for Gemini`)
      } else if (response.status === 401 || response.status === 403) {
...
Keep it relevant for rural Indian villages.`
  ...
  // Analyze health symptoms (Triage)
  async analyzeSymptoms(symptoms: string, age: string, gender: string, existingConditions: string): Promise<AIResponse> {
    const prompt = `Analyze these health symptoms and provide a triage assessment:
...
1. **Potential Causes**: List 3-5 possible causes, sorted by likelihood.
2. **Severity Assessment**: Rate as Low, Medium, High, or Critical Emergency.
3. **Immediate Actions**: What should the patient do right now (e.g., rest, drink water, go to hospital).
4. **Home Remedies**: Safe remedies if applicable for mild cases.
5. **Medical Advice**: When to see a doctor or go to the emergency room.

Disclaimer: State clearly that this is an AI assessment and not a replacement for professional medical advice.

Keep the language simple and easy to understand for a village setting.`

    try {
      return await this.chat(prompt, 'Health symptom triage and analysis')
    } catch (error) {
      return {
        content: `⚠️ **Health Analysis Unavailable**

I cannot properly analyze your symptoms right now due to technical difficulties.

**General Safety Guidelines:**
1. If you have severe pain, difficulty breathing, or high fever, **seek medical attention immediately**.
2. Call **108** for an ambulance if it is an emergency.
3. Visit your nearest Village Health Center.
