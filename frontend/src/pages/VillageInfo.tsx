...
const VillageInfo = () => {
  ...
  useEffect(() => {
    if (socket && isConnected) {
      // Refresh data on any major village event
      const handleRefresh = () => {
        fetchVillageData()
      }

      socket.on('dashboardStatsUpdate', handleRefresh) // Reuse stats event
      socket.on('newAlert', handleRefresh)
      socket.on('sosAlert', handleRefresh)
      socket.on('userRegistered', handleRefresh) // Assuming this event exists
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="bg-white/5 backdrop-blur-sm border border-white/10 p-3 sm:p-4 rounded-lg">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center text-white font-bold text-base sm:text-lg flex-shrink-0 ${airQuality.color === 'green' ? 'bg-green-500' :
